...
cov_list = list()
sample_list = list()
center_corr_list = list()
center_member_list = list()
member_list = list()
a_ran_in_list=list()
n=50
heta_data_list=list()
center_corr_list = list()
het_center_list = list()
het_center_member_list=list()
wc_corr_list = list()
het_non_center_list = list()
het_wc_member_list = list()
het_a_ran_in_list = list()
het_wc_a_ran_in_list = list()
for (i in 1:100){
  ### sample data
  ...
  center_corr_list[[i]]<-center_corr
  ...
  center_member_list[[i]]<-center_member
  
  
  member=membership(center_member) 
  member_list[[i]]=member
  
  plot(center_member,network) 
  
  a_ran_in=adj.rand.index(membership(true_member),membership(center_member))
  a_ran_in_list[[i]] = a_ran_in
  ...
  het_center_list[[i]]<-het_center
  het_center_member=membership(het_center)
  het_center_member_list[[i]]<-het_center_member
  ...
  wc_corr_list[[i]]<-wc_corr
  
  r = wc_corr[order(wc_corr[1:50,1:50], decreasing=TRUE)]
  r
  eta = r[cut_point]
  eta
  
  wc_corr[wc_corr<eta] <- 0
  wc_corr[wc_corr>eta] <- 1
  ...
  het_non_center_list[[i]]<-het_non_center
  het_wc_member_list[[i]]=membership(het_non_center)
  
  plot(het_non_center,wc_network)
  plot(het_center,network)
  
  het_a_ran_in=adj.rand.index(membership(true_member),membership(het_center))
  het_a_ran_in_list[[i]]<-het_a_ran_in
  het_wc_a_ran_in=adj.rand.index(membership(true_member),membership(het_non_center))
  het_wc_a_ran_in_list[[i]]<-het_wc_a_ran_in
}
cov_list
sample_list
center_corr_list
...
center_corr_list
het_center_list
het_center_member_list
wc_corr_list
het_non_center
het_wc_member_list
het_a_ran_in_list
het_wc_a_ran_in_list

### boxplot
dev.off()
List = list(a_ran_in_list,het_a_ran_in_list,het_wc_a_ran_in_list)   
List
df<-data.frame(List=unlist(List),Group=rep(c("Homo","CSE Hetero","Non-CSE Hetero")))
df
boxplot(List ~ Group, data = df)

### t-test
d <- as.numeric(a_ran_in_list)- as.numeric(het_a_ran_in_list)
d
res <- t.test(d)
res
res$p.value

d1 <- as.numeric(het_a_ran_in_list)- as.numeric(het_wc_a_ran_in_list)
d1
res1 <- t.test(d1)
res1
res1$p.value 

d2 <- as.numeric(a_ran_in_list)- as.numeric(het_wc_a_ran_in_list)
d2
res2 <- t.test(d2)
res2
res2$p.value
